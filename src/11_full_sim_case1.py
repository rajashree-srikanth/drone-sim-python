...
phase 1 - formation goal - square through gvf circular formation
        ...
        - condition to exit phase 1: when all 4 aircraft are very close to the required final states:
...
phase 2 - formation goal -> square + tracking to get to final formation
        ...
        - t_opt = 6s - trajectory is a straight line
        - optimization computation time is low (approx. 0.6s)

phase 3 - tracking of pref-defined traj - infinity loop
        - uses diff flatness controller
        - the tracking of this trajectory is performed until total simulation time exceeds 200s
...
input parameters:
                ...
                X1_f - formation states desired at end of phase I
                (achieving these states triggers the initialization of phase II)
...
def CircularFormationGVF(c, r, v, n_ac, X0f, t_start=0, t_step=0.05, t_end=1000):
    t_f = t_end
...
def main():
    ...
    t_end_1 = 1000 # arbitrary - phase I stops when required circle formation is achieved
    ...
    #### phase 1 of flight ####
    print(t_end_1)
    X_array_1, U_array_1, U1_1, U2_1, Ur_1, e_theta_arr_1, time_1, t1_f = CircularFormationGVF(c, r, v, n_ac, X1_f, t_start, t_step, t_end_1)
